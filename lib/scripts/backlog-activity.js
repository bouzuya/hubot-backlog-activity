...
module.exports = function(robot) {
  var API_KEY, INTERVAL, MAPPINGS, SPACE_ID, USER_MAPPINGS, USE_SLACK, fetchActivity, fetchAndSendActivity, fetchResolutions, fetchStatuses, formatActivity, get, getMention, getResolution, getStatus, minId, resolutions, rpad, sendActivity, statuses, _ref, _ref1, _ref2;
  ...
  getMention = function(activity) {
    var assigner, c, mentionName, mentionNames, _ref3;
    c = activity.content;
    assigner = (_ref3 = c.changes) != null ? _ref3.filter(function(change) {
  ...
  sendActivity = function(robot, activity) {
    var mention, message, reply_to, room, rooms, user, wrapped, _ref3, _ref4;
    rooms = JSON.parse(MAPPINGS);
    room = rooms[activity.project.projectKey];
    if (room == null) {
      return;
    }
    if ((_ref3 = activity.type) !== 1 && _ref3 !== 2 && _ref3 !== 3) {
      return;
    }
    message = formatActivity(activity);
    wrapped = USE_SLACK ? '```\n' + message + '\n```' : message;
    mention = getMention(activity);
    if ((mention != null ? mention.length : void 0) > 0) {
      if (USE_SLACK) {
        reply_to = (_ref4 = robot.brain.userForName(mention)) != null ? _ref4.reply_to : void 0;
        user = {
          name: mention
        };
        return robot.send({
  ...
  fetchStatuses = function(callback) {
    return get('/api/v2/statuses', callback);
  };
  fetchResolutions = function(callback) {
    return get('/api/v2/resolutions', callback);
  };
